...
def LUT_Building(SumArr: np.ndarray) -> (np.ndarray):
    ...
    for i in range(len(SumArr)):
        LUTarr[i]=math.ceil((SumArr[i]*255)/(SumArr[len(SumArr)-1])) # exactly the same formula as written in the PDF
    return LUTarr
...
def convertToImg(imOrig: np.ndarray, histOrig: np.ndarray, imYIQ: np.ndarray, arrayQuantize: np.ndarray) -> (np.ndarray, float):
    """
